...
\emph{	Une centrale à inertie ou centrale inertielle est un instrument utilisé en navigation, capable d'intégrer les mouvements d'un mobile (accélération et vitesse angulaire) pour estimer son orientation (angles de roulis, de tangage et de cap), sa vitesse linéaire et sa position. L'estimation de position est relative au point de départ ou au dernier point de recalage.}\footnote{définition de Wikipedia} \end{quote}
	...
	\paragraph{}
	La précision d'une centrale inertielle varie en fonction de plusieurs paramètres: la précision et la calibration des capteurs (accéléromètres et gyroscopes) utilisés mais aussi en fonction de l'algorithme de calcul utilisé pour traiter les données.
...
\chapter{Réalisation du projet}
	...
	\paragraph{}
	Avant de vouloir traiter une information, il est important de la mesurer avec précision via un capteur, puis de la stocker en mémoire pour l'utiliser plus tard dans les calculs.
	...
	C'est la fonction qui est appelée pendant l'initialisation du programme. Elle permet de configurer correctement les différents capteurs utilisés plus tard dans le programme.\\
	Il est possible de jouer sur des paramètres comme le mode d'alimentation, l'échelle utilisée, ou encore la configuration du filtrage des signaux des capteurs.
	...
	\paragraph{}
	Concernant le gyroscope, le principe est exactement le même avec les fonctions \emph{InitGyro(void)} et \emph{ReadGyro(float* pfData)}.
  ...
  \subsection{Traitements des données}
	...
	Cette fonction permet d'effectuer le changement de repère du vecteur position de la carte. Elle calcule la position de la carte dans le repère absolu connaissant son déplacement dans son repère propre et les angles de roulis, tangage et lacet de la carte.
	...
	La librairie permettant d'envoyer et de recevoir des message à travers une liaison série est également mise à disposition par \textsc{STMicroelectronics} dans le projet de démonstration. \\
	...
	\paragraph{}
	L'interface utilisateur a été réalisée en \emph{C++}, à l'aide du framework \emph{Qt}. Cette solution permet une utilisation multiplateforme de l'application, une grande liberté de programmation ainsi qu'une communauté très active.
...
\chapter{Ce qu'il reste à faire et améliorations}
