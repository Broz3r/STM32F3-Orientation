...
\chapter{Réalisation du projet}
  ...
  \subsection{Traitements des données}
	...
	Les calculs de les angles de roulis et de tangage fonctionne très bien et donnent des résultats fiables et utilisables. Le problème vient du calcule de l'angle de lacet qui n'est précis qu'à l'horizontal. Une fois la carte dans une orientation quelconque, la valeur calculée de l'angle n'est plus correcte et en peut alors plus être utilisée ensuite, l'erreur ce répercutant dans les couches supérieures.
	
	\subsubsection*{EulerMethode(float *primitive, float *function, int delay)}
	C'est la fonction qui permet de retourner dans le pointeur de la primitive en paramètre le calcul de cette primitive en fonction de sa fonction de référence \emph{function} et de la durée de entre deux mesure \emph{delay} par la méthode d'Euler :
	
	\begin{figure}[h]
	\centering
		\includegraphics[scale=0.7]{eulerMethode.png}
	\caption{Fonction void EulerMethode(float *primitive, float *function, int delay)}
	\end{figure}
	...
	\paragraph{}
	L'interface utilisateur a été réalisée en \emph{C++} à l'aide du farmework \emph{Qt} qui permet une utilisation multiplateforme et une grande liberté de programmation tout en ayant une communauté très active.
	...
	\paragraph{}
	Si les paramètres rentré par l'utilisateur sont correct, il est alors possible de se connecter à la liaison série. Il faut tout fois que la liaison série ne soit pas déjà connecté à un autre programme, comme un hyper terminal, puisque la liaison série ne permet pas le multicast.\\
	Une fois connectée, l'interface récupère les messages envoyé par la carte sur la liaison série et concatène les données pour les réinjecter dans les éléments de l'interface graphique.
	...
	Les angles de roulis, tangage et lacet sont indiqués par les afficheur LCD en haut de l'interface alors que la partie centrale permet quand à elle d'orienter un parallélépipède rectangle en fonction de l'orientation de la carte. Toute la partie 3D est réalisé à l'aide des librairies \emph{QtOpenGL} et \emph{QGLWidget}.
...
\chapter{Reste à faire et améliorations}
	...
	\paragraph{} Toute fois cette méthode n'est pas non plus parfaite puisqu'elle est amputée d'une erreur de \emph{dérive} que le gyroscope ne peut mesurer. Il faudrait alors mettre en abîme les deux méthodes de calcul afin que l'une compense l'erreur de l'autre et vice et versa. 
	...
	\paragraph{}
	D'autre méthode pourrait être aussi implémentée, comme la méthode du filtre de Kalman ou le filtre de Madgwick utilisée dans les centrales inertielles industrielles.
	
	\section{Communication Interface vers carte}
	\paragraph{}
	On pourrait utiliser la liaison série dans le sens inverse. C'est-à-dire au lieu de faire du Broadcast pur et simple de l'information, l'interface utilisateur pourrait aussi écrire sur la liaison série et communiquer avec la carte pour par exemple lui demander d'utiliser une méthode plutôt qu'un autre ou modifier le temps de cycle de la carte.
...
\paragraph{}
Cela amorce les prémisses d'une centrale inertielle, mais comme nous l'avons vu, beaucoup de chemins et d'améliorations restent encore à faire, principalement en matière d'algorithme.\\
